...
HVAC failure (simulate_fan_failure) anomaly without retraining?

Run with:
    PYTHONPATH=. python backend/tests/test_hvac_feasibility.py
"""
...
def main():
    ...
    # ── Phase B — HVAC failure injection ─────────────────────────────────────
    print()
    print("=" * 62)
    print("PHASE B — HVAC failure injection (40 steps)")
    print("=" * 62)
    ...
    if first_detection_step is not None:
        print(
            "  VERDICT: DETECTABLE — model catches hvac_failure without retraining"
        )
    else:
        print(
            "  VERDICT: NOT DETECTABLE — model does not trigger on HVAC failure,"
            " retraining required"
        )
    print()
